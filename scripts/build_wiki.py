#!/usr/bin/env python3
"""
Deterministic markdown wiki generator for the AIEWF 2026 corpus.
...
Publishing posture (binding, see HANDOFF.md): derived layer only — summaries,
...
def render_concept(cslug, ctx):
    ...
    if passC is None:
        lines.append('> Synthesis not yet available for this concept.')
        lines.append('')
    else:
...
README_TEMPLATE = """# AI Engineer World's Fair 2026 — Wiki

A linked, graph-style wiki distilled from every session at the AI Engineer World's Fair
2026 (Moscone West, San Francisco, June 29 – July 2, 2026). {talk_count} talks, transcribed
...
- **{talk_count} talks** ({real_talk_count} individual talks + 2 full-day livestream compilations)
- **{word_count:,} deduplicated words**, **{hours:.1f} hours** of source material
  (raw totals across all {talk_count} entries are {raw_word_count:,} words / {raw_hours:.1f} hours;
  the difference is ~{dup_words:,} words of livestream-compilation content that duplicates
  individual talks already covered separately — kept as its own entry but not double-counted
  in the corpus total above)
- **{speaker_count} speakers**
- **{concept_count} concepts**, each with a cross-talk synthesis: state of practice, consensus,
...
**Speaker attribution:** {multi_speaker_count} talks have multiple speakers. Where the
...
wiki/
  ...
  speakers/<slug>.md    {speaker_count} speaker pages — role, company, bio, talks, sessions,
                        concepts, quotes
```
...
not edited (see `data/passA/_dropped_quotes.json` for the list). {quote_count:,} quotes
passed and are rendered across the {talk_count} talk pages ({attributed_quote_count:,}
individually speaker-attributed, {talk_level_quote_count:,} talk-level from multi-speaker
sessions).
...
def render_readme(ctx, real_talk_count, multi_speaker_count):
    ...
    for cslug, passC in ctx['passC_by_slug'].items():
        if passC and passC.get('maturity'):
            maturity_counts[passC['maturity']] += 1

    return README_TEMPLATE.format(
        talk_count=talk_count,
        real_talk_count=real_talk_count,
...
def build(root):
    ...
    real_talk_count = sum(1 for s in ctx['talks_by_slug'] if 'wf2026-' not in s)
