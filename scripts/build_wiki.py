...
A linked, graph-style wiki built from {talk_count} talks at the AI Engineer World's Fair
...
sponsor slots. This corpus covers the {talk_count} that had been published to the
...
- **{talk_count} talks** ({real_talk_count} individual talks plus 2 full-day livestream compilations)
- **{word_count:,} deduplicated words**, **{hours:.1f} hours** of source material.
  Raw totals across all {talk_count} entries are {raw_word_count:,} words and
  {raw_hours:.1f} hours. The difference is roughly {dup_words:,} words of
...
- **{concept_count} concepts**, each with a cross-talk synthesis: state of practice,
