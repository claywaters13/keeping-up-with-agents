#!/usr/bin/env python3
"""
Build a speaker index joining raw/speakers.json profiles to data/index.json talks
(via data/passA/<slug>.json extractions) and raw/sessions.json schedule entries.
...
def build(root: str):
    ...
    with open(os.path.join(raw_dir, 'speakers.json')) as f:
        profiles_raw = json.load(f)['speakers']
    with open(os.path.join(raw_dir, 'sessions.json')) as f:
